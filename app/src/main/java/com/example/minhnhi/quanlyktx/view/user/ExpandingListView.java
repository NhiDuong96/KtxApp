...
public class ExpandingListView extends ListView {
     ...
     * its current state.
     */
    private OnItemClickListener mItemClickListener = (parent, view, position, id) -> {
        ExpandableNotificationItem viewObject = (ExpandableNotificationItem)getItemAtPosition(getPositionForView
                (view));
        if (!viewObject.isExpanded()) {
            expandView(view, viewObject);
        } else {
            collapseView(view, viewObject);
        }
    };
    ...
    private void expandView(final View view, final ExpandableNotificationItem viewObject) {
